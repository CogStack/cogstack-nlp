...
class Linker(AbstractEntityProvidingComponent):
    ...
    def __init__(self, cdb: CDB, config: Config) -> None:
        ...
        self.context_model = ContextModel(self.config, self.cnf_l, self.cdb, self._name_keys, self._cui_keys)
    ...
    def create_embeddings(self,
        ...
        self.context_model.embed_cui_names(embedding_model_name)
    ...
    def _get_context_vectors(
        ...
        for entity in entities:
            ...
            texts.append(text)
        return self.context_model.embed(texts, self.device)
    ...
    def _build_context_matrices(self) -> None:
        if "name_embeddings" in self.cdb.addl_info:
            self._names_context_matrix = (
                self.cdb.addl_info["name_embeddings"].to(self.device)
            )
        if "cui_embeddings" in self.cdb.addl_info:
            self._cui_context_matrix = (
                self.cdb.addl_info["cui_embeddings"].to(self.device)
            )
    ...
    def _generate_link_candidates(
        ...
        for i, entity in enumerate(entities):
            ...
            top_idxs = row_sorted[:top_n].tolist()
            top_names = [self._name_keys[idx] for idx in top_idxs]
            top_scores = names_scores[i, top_idxs].tolist()
            entity_text = doc.base.text[entity.base.char_index:entity.base.char_index + len(entity.base.text)]
            print(
                "Entity '%s' top-%d name matches: %s",
                entity_text, top_n,
                list(zip(top_names, [f'{s:.4f}' for s in top_scores]))
            )
            import sys
            sys.exit()
            cuis: set[str] = set()
            ...
            if self.cnf_l.short_similarity_threshold > 0.0:
    ...
    def _pre_inference(self, doc: MutableDocument
        ...
        if not self.cnf_l.use_ner_link_candidates:
            ...
            for entities in self._batch_data(
                all_ents, self.cnf_l.linking_batch_size
            ):
        ...
        for entity in filtered_ents:
            if len(entity.link_candidates) == 1:
                ...
                if self.cnf_l.filters.check_filters(entity.link_candidates[0]):
                    ...
                    # print(entity)
                    # print(self.cdb.cui2info[entity.cui])
    ...
    def predict_entities(self, doc: MutableDocument,
        ...
        # print(self.context_model.embedding_model.use_projection)
        # print(self.context_model.embedding_model)
